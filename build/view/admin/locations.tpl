    ...
    foreach ($entreprises as $entreprise) {
        ...
        echo '<div class="table-responsive border rounded" style="margin-top: 16px;">';
            echo '<table class="table table-striped" style="margin: 0">';
                echo '<thead><tr>';
                    echo '<th class="text-center">Employé</th>';
                    echo '<th class="text-center">Photo</th>';
                    echo '<th class="text-center">Modèle</th>';
                    echo '<th class="text-center">Date de début</th>';
                    echo '<th class="text-center">Date de fin</th>';
                ...
                foreach ($entreprise["employes"] as $employe) {
                    foreach ($employe["factures"] as $facture) {
                        echo '<tr>';
                        printf('<td class="text-center align-middle"><p style="margin: 0;">%s</p></td>', htmlspecialchars($employe["nom"]));
                        printf('<td class="d-xl-flex justify-content-xl-center"><img class="img-fluid" src="./assets/vehicule/img/%s" alt="%s" width="200px" height="100px"></td>', $facture["photo"], $facture["photo"]);
